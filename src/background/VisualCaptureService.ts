...
import { waitForPageReadiness, type PageReadinessState } from '@/background/utils/PageLoadDetector';
...
export class VisualCaptureService {
   ...
   */
  async captureWhenReady(tabId: number): Promise<{ screenshot: string | null; readiness: PageReadinessState }> {
    try {
      // Wait for page to be ready using the utility
      const readiness = await waitForPageReadiness(tabId);
